> Part of [RemiLink Strategy](../index.md). Updated 31.03.2026.

# Gaps and Open Items

Pre-launch checkpoint. Check before making content or design decisions.

## Honest Gaps
1. **Thin social proof** — Most case studies under NDA; most experience in classical ML/CV/NLP, not LLM/agents
2. **EdTech expertise unclear** — "We are not really experts in this sphere"
3. **No professional team photos** — Founder photo available, team photos not

## Open Items

### Unresolved
- [ ] How hard to lead with EdTech
- [ ] Post-first-contact sales pipeline definition
- [ ] Which persona accounts to create first
- [ ] Whether ISO/IEC compliance language helps or distracts
- [ ] Whether to offer Discovery artifacts as secondary CTA
- [ ] Clarify brandbook status (are existing .docx files current?)
- [ ] Review/update existing 4 blog articles
- [ ] CTA wording has to be still improved
- [ ] Vendor selection process research — how do enterprise buyers actually choose AI vendors? (see [icp.md](icp.md#vendor-selection-process))
- [ ] US/EU pricing framing — how to position $15K/mo as a deal (hiring a team costs 3-5x more)
- [ ] Sales pipeline draft (in gtm.md) — needs founder review and refinement

### Resolved
- [x] Company-size target → complexity over headcount
- [x] Public service labels → 5-phase journey
- [x] Pricing display → monthly rates
- [x] Starting monthly rate → $15,000–18,000/month
- [x] Team presentation → founder-first, roles not faces
- [x] Whether and when the "agentic POC" becomes a real offer → yes, it's specified in the positioning document

## What's Done

- [x] Strategy and positioning documented
- [x] Service labels decided
- [x] ICP defined
- [x] Pricing display approach decided
- [x] Team presentation approach decided
- [x] Competitor analysis

## What NOT to Spend Time on Now

- Picking new industries (prove current ones first)
- Hiring a salesperson (can't brief them until positioning is battle-tested)
- Social media beyond LinkedIn
- Computer Vision as a service line
- Complex CRM integration
